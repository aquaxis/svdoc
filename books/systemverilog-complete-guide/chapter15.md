...
また、デバッグはVLSI設計フローにおいて最も多くの時間を費やす工程の一つです。`$display`系タスクによるテキストベースのデバッグから、波形解析、アサーションベースのデバッグ、UVMのレポーティング機能まで、体系的なデバッグ手法を習得します。効率的なデバッグ環境の構築方法についても解説し、チーム開発での生産性向上を目指します。
...
### 15.5.4 条件付きデバッグメッセージ（verbosityレベル）
...
### 15.5.5 `ifdef/`ifndef によるデバッグコードの制御
...
### 15.8.5 自動回帰テストのデバッグ戦略

CI/CD環境での大量テスト実行時に、効率的にデバッグを進めるための戦略を解説します。

![デバッグ戦略フロー](/images/ch15_debug_strategy.drawio.png)
